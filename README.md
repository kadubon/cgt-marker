...
`cgt-marker` is a lightweight contradiction-marker ledger for long-running AI
agents and agent-like workflows.

It records claims, detects simple conflicts, creates typed markers, preserves
provenance, and renders unresolved markers for prompts or downstream state. It does
not decide what is true. It does not resolve contradictions automatically. It does
not force global consistency.
...
explicit instead of being overwritten or hidden in a summary.
...
- `ExactSlotConflictDetector`: same subject and predicate, incompatible values for
  predicates such as `equals`, `is`, `status`, `date`, and `value`.
...
### Full local experiment result

A full local run was performed on 2026-05-17 with `ollama gemma4:e4b`, `trials=1`,
all 5 scenarios, 2 conditions, `temperature=0`, `seed=7`, `num_predict=256`, and
`think=false`. This produced 10 records: 8 conflict records and 2 non-conflict
control records.

Reproduction command:

```powershell
uv run python experiments/marker_context_effect/run_experiment.py --model gemma4:e4b --trials 1 --output experiments/results/marker_context_effect_full.jsonl --manifest-output experiments/results/marker_context_effect_full.manifest.json
```

Public report command:

```powershell
uv run python experiments/marker_context_effect/report.py --input experiments/results/marker_context_effect_full.jsonl --output-md experiments/reports/marker_context_effect_full.md --output-json experiments/reports/marker_context_effect_full.summary.json
```

The primary proxy outcome was:

```text
marker_success = conflict scenario
  AND asserts_visible_conflict
  AND mentions_both_values
...
at the 4 conflict scenarios, `marker_aware` satisfied the primary outcome in 4/4
cases, while `baseline` satisfied it in 0/4 cases.

Interpretation: in this small fixed scenario set, adding rendered marker context
made `gemma4:e4b` more likely to explicitly mention unresolved contradiction,
conflicting values, and sources. The baseline sometimes used conflict-like language,
but did not include both conflicting values and sources, so it did not satisfy the
primary outcome. The control scenario produced no positive visible-conflict
assertion under either condition in this run.
...
- [Full Markdown report](experiments/reports/marker_context_effect_full.md)
- [Full JSON summary](experiments/reports/marker_context_effect_full.summary.json)
